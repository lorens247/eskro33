@if($seo)
    ...
    <link rel="shortcut icon" href="{{ getImage(fileManager()->logoIcon()->path .'/favicon.png') }}" type="image/x-icon">

    {{--<!-- Apple Stuff -->--}}
    <link rel="apple-touch-icon" href="{{ getImage(fileManager()->logoIcon()->path .'/logo.png') }}">
    ...
    <meta itemprop="image" content="{{ getImage(fileManager()->seo()->path .'/'. $seo->image) }}">
    {{--<!-- Facebook Meta Tags -->--}}
    <meta property="og:type" content="website">
    <meta property="og:title" content="{{ $seo->social_title }}">
    <meta property="og:description" content="{{ $seo->social_description }}">
    <meta property="og:image" content="{{ getImage(fileManager()->seo()->path .'/'. $seo->image) }}"/>
    <meta property="og:image:type" content="image/{{ pathinfo(getImage(fileManager()->seo()->path) .'/'. $seo->image)['extension'] }}" />
    @php $social_image_size = explode('x', fileManager()->seo()->size) @endphp
