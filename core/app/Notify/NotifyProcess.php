...
class NotifyProcess{
    ...
    */
	protected function replaceShortCode($name,$username,$template,$body){
		$message = shortCodeReplacer("{{fullname}}", $name, $template);
	    $message = shortCodeReplacer("{{username}}", $username, $message);
	    $message = shortCodeReplacer("{{message}}", $body, $message);
	    return $message;
	}
    ...
    */
	public function createLog($type){
		...
		if ($this->user && $this->createLog) {
			...
			$emailLog = new NotificationLog();
		    $emailLog->$userColumn = $this->user->id;
		    $emailLog->type = $type;
		    $emailLog->sender = @$config->name;
		    $emailLog->sent_from = $this->setting->email_from;
		    $emailLog->sent_to = $this->toAddress;
		    $emailLog->subject = $this->subject;
		    $emailLog->message = $type == 'email' ? $this->finalMessage : strip_tags($this->finalMessage);
		    $emailLog->save();
		}
	}
