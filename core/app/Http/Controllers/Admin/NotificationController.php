...
class NotificationController extends Controller
{
    ...
    public function globalUpdate(Request $request){
        ...
        $general->telegram_body = $request->telegram_body;
        $general->save();
    ...
    public function templates(){
        ...
        $emptyMessage = 'No notification templates available';
        $templates = NotificationTemplate::orderBy('name')->get();
        return view('admin.notification.templates',compact('pageTitle','templates','emptyMessage'));
    }
    ...
    public function templateEdit($id)
    {
        ...
        return view('admin.notification.edit', compact('pageTitle', 'template','emptyMessage'));
    }
    ...
    public function emailTest(Request $request){
        ...
        $message = 'Your email notification setting is configured successfully for '.$general->sitename;
    ...
    public function smsTest(Request $request){
        ...
        if ($general->sn == 1) {
            ...
            $sendSms->message = 'Your sms notification setting is configured successfully for '.$general->sitename;
            $sendSms->subject = ' ';
            $sendSms->send();
        }else{
            $notify[] = ['info', 'Please enable from general settings'];
    ...
    public function telegramSetting(){
        $pageTitle = 'Telegram Notification Settings';
    ...
    public function telegramSettingUpdate(Request $request){
        ...
        $general->save();
        $notify[] = ['success', 'Telegram notification settings updated successfully'];
        return back()->withNotify($notify);
    }

    public function telegramTest(Request $request){
        $request->validate(['telegram_username' => 'required']);
        $general = GeneralSetting::first();
        if ($general->tn == 1) {
            $sendTelegram = new Telegram;
            $sendTelegram->telegramUsername = $request->telegram_username;
            $sendTelegram->receiverName = ' ';
            $sendTelegram->message = 'Your telegram notification setting is configured successfully for '.$general->sitename;
            $sendTelegram->subject = ' ';
            $sendTelegram->send();
        }else{
            $notify[] = ['info', 'Please enable from general settings'];
            $notify[] = ['error', 'Your telegram notification is disabled'];
            return back()->withNotify($notify);
        }

        if (session('telegram_error')) {
            $notify[] = ['error', session('telegram_error')];
        }else{
            $notify[] = ['success', 'Telegram sent to ' . $request->telegram_username . ' successfully'];
        }
