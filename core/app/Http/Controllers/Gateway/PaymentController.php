...
class PaymentController extends Controller
{
    ...
    public function depositInsert(Request $request)
    {
        ...
        $request->validate([
            ...
            'currency' => 'required',
            'type' => 'required|in:checkout,deposit',
        ]);
        ...
        if ($request->type == 'checkout') {
            ...
            if (!$checkOutData && ($request->amount != $checkOutData['amount'])) {
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            if ($data->milestone_id) {
                Milestone::makePaid($data->milestone_id,$user);
            }
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        $path = fileManager()->verify()->deposit->path.'/'.$directory;
                        ...
                        if ($inVal->type == 'file') {
                            if ($request->hasFile($inKey)) {
                                try {
                                    ...
                                    $notify[] = ['error', $inKey.' file couldn\'t upload'];
        ...
        $adminNotification->title = 'New deposit request from '.$data->user->username;
        ...
        $notify[] = ['success', 'Deposit request sent successfully.'];
